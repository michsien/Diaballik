...
begin
  ...
  else ShowMessage('This already is the initial setup.');
end;
...
begin
  if NumerRuchu<LicznikRuchow then
    ...
    ShowMessage('This already is the situation after the last move.');
end;
...
begin
  if NumerRuchu=LicznikRuchow then
    begin
      ShowMessage('Go back before replaying;)');
      Exit;
    end;
  Odtwarzamy:=true;
  ZablokujGuziki();
...
begin
  ...
  if Pom=0 then
    ShowMessage('This already is the initial setup;)')
  else
    Skocz(-Pom)
end;

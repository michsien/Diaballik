...
jeœli biezacy gracz to nie Cz³owiek1/Komputer1 to odwracamy uklad pionkow
...
begin
  ...
  if (EkranGlowny.WpiszBiezacyGracz.Text='Cz³owiek1') or
...
begin
  ...
  for i:=1 to MAX_RUCHOW do
    if ObecneRuchy[i].Uzyty then
      Inc(IleRuchow);
  Hint:='Zrob '+IntToStr(IleRuchow)+' ruchy.';
  for i:=1 to MAX_RUCHOW do
    if ObecneRuchy[i].Uzyty then
      Hint:=Hint+' Z '
            +WspolrzedneNaOpis(ObecneRuchy[i].Wiersz1,ObecneRuchy[i].Kol1)+' do '

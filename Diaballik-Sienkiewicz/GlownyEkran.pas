...
const
  ...
  MA_PODANIE='Pozosta³o mu 1 podanie.';
  NIE_MA_PODAN='Wykorzysta³ swoje podanie!';
...
procedure PrzygotujKomponentyUruchomKomputer();
begin
  if (EkranGlowny.WpiszBiezacyGracz.Text='Komputer1') or
    (EkranGlowny.WpiszBiezacyGracz.Text='Komputer2') then
    begin
      ZablokujKomponenty();
...
begin
  ...
  if (not Gramy) or (WpiszBiezacyGracz.Text='Komputer1') or
    (WpiszBiezacyGracz.Text='Komputer2')  then
    Exit;
  ...
  if WpiszBiezacyGracz.Text='Cz³owiek1' then
    begin
      Pion:=PION_1;
      Pilka:=PILKA_1;
    end
...
begin
  ...
  if ListaWyborTrybu.ItemIndex=-1 then
    begin
      ShowMessage('Niepoprawny tryb!!');
      Exit;
    end
  else if Gramy then
    if MessageDlg('Obecna gra zostanie stracona!', mtConfirmation,
  ...
  Case ListaWyborTrybu.ItemIndex of
    0 : begin
          NapisNazwaGracza1.Caption:='Cz³owiek1';
          NapisNazwaGracza2.Caption:='Cz³owiek2';
          WpiszBiezacyGracz.Text:='Cz³owiek1'
        end;
    1 : begin
          NapisNazwaGracza1.Caption:='Cz³owiek1';
          NapisNazwaGracza2.Caption:='Komputer2';
          WpiszBiezacyGracz.Text:='Cz³owiek1'
        end;
    2 : begin
          NapisNazwaGracza1.Caption:='Komputer1';
          NapisNazwaGracza2.Caption:='Cz³owiek2';
          WpiszBiezacyGracz.Text:='Komputer1'
        end;
    3 : begin
          NapisNazwaGracza1.Caption:='Komputer1';
          NapisNazwaGracza2.Caption:='Komputer2';
...
begin
  ...
  if IleRuchow=0 then
    begin
      ShowMessage('Nie wykonano ¿adnych ruchów!');
      Exit;
    end;
  ...
  if (WpiszBiezacyGracz.Text='Cz³owiek1') or (WpiszBiezacyGracz.Text='Komputer1')
    then
    begin
      Gracz:=1;
  ...
  if CzyWygrana(Gracz,PionkiObecnie) then
    begin
      ShowMessage('Przy³o¿enie! Zwyciê¿y³ gracz '+NazwaGracz+' !!');
      ZablokujKomponenty;
      Exit;
    end;
  if CzyWygranaPrzezBlokade(Przeciwnik,PionkiObecnie,Linia) then
    begin
      ShowMessage(NazwaGracz+' stworzy³ blokadê! Zwyciê¿y³ gracz '+
                  NazwaPrzeciwnik+' !!');
      ZablokujKomponenty();
      Exit;
    end;
  if CzyWygranaPrzezBlokade(Gracz,PionkiObecnie,Linia) then
    begin
      ShowMessage(NazwaGracz+' napotka³ blokadê! Zwyciê¿y³ gracz '+
                  NazwaGracz+' !!');
      ZablokujKomponenty();
      Exit;
    end;
...
begin
  if not Gramy then
    begin
      ShowMessage('By mo¿na by³o u¿yæ edytora, nale¿y uruchomiæ jak¹œ grê!');
      Exit;
    end;
...
begin
  if not Gramy then
    begin
      ShowMessage('Jeœli chcesz zapisaæ grê to musisz najpierw zacz¹æ graæ.');
      Exit;
    end;
...
begin
  if not Gramy then
    begin
      ShowMessage('By przejrzeæ historiê gry, najpierw w jak¹œ zagraj!');
      Exit;
    end;
...
begin
  if (WpiszBiezacyGracz.Text='Komputer1') or
    (WpiszBiezacyGracz.Text='Komputer2') then
    begin
      ShowMessage('Teraz gra komputer. Hinty s¹ przeznaczone dla ludzi.');
      Exit;
    end
  else if MessageDlg('Komputer poda Ci propozycjê ruchów, które mo¿esz wykonaæ w'
    +' tej kolejce. Za karê, na chwilê program mo¿e nie reagowaæ na Twoje'+
    ' klikanie. Zgadzasz siê?', mtConfirmation, [mbOk,mbCancel], 0) = mrCancel
    then
    Exit;
